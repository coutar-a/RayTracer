...
** Last update Sun Jun  8 11:17:17 2014 grelli_t
*/
...
double		sphere_int_shadow(t_3d *eye, t_3d *vc, t_objs *sph)
{
  ...
  c = (pow(eye->x, 2.0) + pow(eye->y, 2.0) + pow(eye->z, 2.0) +
	...
	2.0 * (sph->pos[0] * eye->x + sph->pos[1] * eye->y + sph->pos[2] *
	       eye->z) -
	pow(sph->ray, 2.0)));
...
double		plan_int_shadow(t_3d *eye, t_3d *vc, t_objs *pl)
{
  ...
  test = -(eye->x * pl->pos[0] + eye->y * pl->pos[1] + eye->z *
	   pl->pos[2] + pl->ray) / (pl->pos[0] * vc->x + pl->pos[1] * vc->y +
				    pl->pos[2] * vc->z);
...
double		cone_int_shadow(t_3d *eye, t_3d *vc, t_objs *cone)
{
  ...
  d.a = pow(vc->x, 2.0) + pow(vc->y, 2.0) - (q * pow(vc->z, 2.0));
  d.b = 2.0 * (vc->x * (eye->x - cone->pos[0]) + vc->y *
	      (eye->y - cone->pos[1]) - q * vc->z * (eye->z - cone->pos[2]));
  d.c = pow(eye->x, 2.0) + pow(cone->pos[0], 2.0) + pow(eye->y, 2.0) +
    pow(cone->pos[1], 2.0) - q * pow(eye->z, 2.0) - q *
    pow(cone->pos[2], 2.0) - 2 * (cone->pos[0] * eye->x + cone->pos[1] *
			      eye->y - (q * cone->pos[2] * eye->z));
  d.delta = pow(d.b, 2.0) - (4.0 * d.a * d.c);
  ...
  if (d.delta < 0.0)
    ...
    return((-d.b) / (2.0 * d.a));
  d.k1 = ((-d.b) - (sqrt(d.delta))) / (2.0 * d.a);
  d.k2 = ((-d.b) + (sqrt(d.delta))) / (2.0 * d.a);
  return (CPR_MIN(d.k1, d.k2));
}
